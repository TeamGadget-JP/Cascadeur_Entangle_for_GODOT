...
Due to a limitation in Godot's C# editor, when you place an avatar in a new scene and attach the script for the very first time, the Inspector values (such as Target Skeleton) are not immediately reflected in the internal system.

After configuring your first avatar, please make sure to save the scene (Ctrl+S), close the scene tab, and reopen it before turning on Connect To Cascadeur.
(Note: When adding the second and subsequent avatars, or after the scene has been reopened once, you can synchronize seamlessly without this step.)
...
step7: 一旦シーンを閉じて、再度シーンを読み込んでください。<br>
...
【初回セットアップ時の注意点】
GodotエンジンのC#エディタ仕様により、新規シーンにアバターを配置して初めてスクリプトをアタッチした直後は、インスペクターの設定値（Target Skeletonなど）が内部システムに即座に反映されません。
初回の1体目をセットアップした後は、必ず一度シーンを保存（Ctrl+S）し、シーンタブを閉じて開き直してから Connect To Cascadeur をONにしてください。
（※2体目以降のアバター追加時や、一度シーンを開き直した後は、そのままシームレスに同期が可能です）
